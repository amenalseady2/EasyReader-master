...
public class XImageView extends android.support.v7.widget.AppCompatImageView {
    ...
    public void setImageURL(String url, int defImgRes) {
        loadImage(url, defImgRes);
    }

    private void loadImage(String url, int defImgRes) {
         if (defImgRes != 0) {
            if(defImgRes != 1){
                Glide.with(mContext).load(url).centerCrop().error(defImgRes).placeholder(defImgRes).into(this);
            }else{
                Glide.with(mContext).load(url).centerCrop().into(this);
            }
        } else {
            Glide.with(mContext).load(url).centerCrop().error(R.mipmap.ic_shoplist_default).placeholder(R.mipmap.ic_shoplist_default).into(this);
        }
    }

    public void loadImageDefault(String url, int defImgRes) {
        Glide.with(mContext).load(url).error(defImgRes).placeholder(defImgRes).into(this);
    }
...
//         *
//         * @param modeInt
//         * @return 设置的大小图，默认返回SMALL
//         * @author mslan
//         * @date 2015-5-27 下午2:07:52
//         */
//        static ImageMode mapIntToValue(final int modeInt) {
//            for (ImageMode value : ImageMode.values()) {
