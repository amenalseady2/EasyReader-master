...
public class XRecycleView extends RecyclerView {
    ...
    //当前的头布局
    private XAnimationHeader mHeadView;
    //监听器
    ...
    private void init() {
        ...
        if (pullRefreshEnabled) {
            //获取头布局
            XAnimationHeader refreshHeader = new XAnimationHeader(getContext());
            //添加进去
            mHeaderViews.add(0, refreshHeader);
            mHeadView = refreshHeader;
        }
        XRecycleVIewFooter footView = new XRecycleVIewFooter(getContext());
    ...
    public void onScrollStateChanged(int state) {
        ...
        if (state == RecyclerView.SCROLL_STATE_IDLE && refreshAndLoadMoreListener != null && !isLoadingData && loadMoreEnabled) {
            ...
            if (layoutManager.getChildCount() > 0
                    ...
                    && !isNoMore && mHeadView.getStatus() != XAnimationHeader.STATE_REFRESHING) {

                View footView = mFootViews.get(0);
                isLoadingData = true;
                if (footView instanceof XRecycleVIewFooter) {
                    ((XRecycleVIewFooter) footView).setState(XRecycleVIewFooter.STATE_LOADING);
                } else {
...
//        adapter.registerAdapterDataObserver(mDataObserver);
        ...
        public void onChanged() {
            ...
            if (adapter != null && mEmptyView != null) {
                ...
                if (adapter.getItemCount() == emptyCount) {
                    ...
                    XRecycleView.this.setVisibility(View.GONE);
                } else {
                    mEmptyView.setVisibility(View.GONE);
                    XRecycleView.this.setVisibility(View.VISIBLE);
                }
            }
    ...
    public void setNoMoreData(boolean noMore) {
        ...
        View footView = mFootViews.get(0);
        ((XRecycleVIewFooter) footView).setState(isNoMore ? XRecycleVIewFooter.STATE_NOMORE : XRecycleVIewFooter.STATE_COMPLETE);
    }
    ...
    public void setRefreshFooter() {
        View footView = mFootViews.get(0);
        ((XRecycleVIewFooter) footView).setState(XRecycleVIewFooter.STATE_REFRESH);
    }
    ...
    public void setFootViewVisible(){
        View footView = mFootViews.get(0);
        footView.getLayoutParams().height = ViewGroup.LayoutParams.WRAP_CONTENT;
    }
    ...
    public void setBackGroundColor(int color) {
        mFootViews.get(0).setBackgroundColor(color);
        mHeaderViews.get(0).setBackgroundColor(color);
    }
    ...
    public void setloadMoreComplete() {
        ...
        View footView = mFootViews.get(0);
        if (footView instanceof XRecycleVIewFooter) {
            ((XRecycleVIewFooter) footView).setState(XRecycleVIewFooter.STATE_COMPLETE);
        } else {
    ...
    public void setloadMoreDone() {
        ...
        View footView = mFootViews.get(0);
        if (footView instanceof XRecycleVIewFooter) {
            ((XRecycleVIewFooter) footView).setState(XRecycleVIewFooter.STATE_NOMORE);
        } else {
            footView.setVisibility(View.GONE);
        }
    }
     ...
     * 设置数据加载完
     */
    public void setloadMoreDone(boolean noFooter) {
        //设置加载数据为false
        isLoadingData = false;
        View footView = mFootViews.get(0);
        if (noFooter) {
    ...
    public void setRefreshing(boolean refreshing) {
        if (refreshing && pullRefreshEnabled && refreshAndLoadMoreListener != null) {
            mHeadView.setState(XAnimationHeader.STATE_REFRESHING);
    ...
    public void addHeadView(View headView) {
        if (pullRefreshEnabled && !(mHeaderViews.get(0) instanceof XAnimationHeader)) {
            XAnimationHeader refreshHeader = new XAnimationHeader(getContext());
            mHeaderViews.add(0, refreshHeader);
            mHeadView = refreshHeader;
        }
     ...
     * @param footView
     */
    private void addFootView(XRecycleVIewFooter footView) {
        mFootViews.clear();
